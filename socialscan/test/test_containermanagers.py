"""
Unit test module for model.containers module (primarily the manager classes)
Run tests by executing on the command line: python test_containermanagers.py
"""
...
# __file__ is <root>/test/test_containermanagers.py
pdn = path.dirname
...
class DigestManagerTest(unittest.TestCase):
    engine = None
    session = None
    siginfo = None
    config = None
    ...
    def setUpClass(cls):
        ...
        owner = owner = Peer('sdmgrtest-owner', 'sdmgrtest-owner', 'sdmgrtest-owner')
    ...
    def testInit(self):
        'Make sure we can create a DigestManager instance.'
        sdmgr = DigestManager(self.config, self.session)
        self.assertEqual(sdmgr.config, self.config)
        self.assertEqual(sdmgr.session, self.session)
        # TODO:
...
def suite():
    sdmanager_suite = unittest.makeSuite(DigestManagerTest)
    slmanager_suite = unittest.makeSuite(ScanLogManagerTest)
    suite = unittest.TestSuite((sdmanager_suite, slmanager_suite))
    return suite
